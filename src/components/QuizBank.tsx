...
import type { QuizQuestion, QuizMode, QuizCategory } from '@/types';
...
export default function QuizBank() {
  ...
  async function toggleBookmark(questionId: string) {
    try {
      ...
      setQuestions((prev) =>
        prev.map((q) =>
          q.id === questionId ? { ...q, is_bookmarked: data.is_bookmarked } : q
        )
      );
    } catch {
      // Silently fail — bookmark is non-critical
    }
  }
  ...
  const emptyMessages: Record<QuizMode, { title: string; body: string }> = {
    ...
    bookmarked: {
      ...
      body: 'Tap the star icon on any question to bookmark it for later review.',
    },
    missed: {
      title: 'No missed questions',
      body: 'Questions you answer incorrectly will appear here so you can retry them.',
    },
  };

  return (
    <div className="space-y-6 max-w-2xl mx-auto">
      {/* Mode & Category filter bar */}
      ...
      {!loading && questions.length === 0 && (
        <div className="flex flex-col items-center justify-center py-20 text-center">
          ...
          <h3 className="text-lg font-medium text-white/50 mb-2">
            {emptyMessages[mode].title}
          </h3>
          <p className="text-sm text-white/30 max-w-md">
            {emptyMessages[mode].body}
          </p>
        </div>
      )}
            ...
            >
              ...
              <div className="flex items-center gap-2 mb-4">
                ...
                <span
                  ...
                  style={{
                    color: CATEGORY_COLORS[question.category] || '#00e5ff',
                    backgroundColor: `${CATEGORY_COLORS[question.category] || '#00e5ff'}15`,
                    border: `1px solid ${CATEGORY_COLORS[question.category] || '#00e5ff'}30`,
                  }}
                >
                  {question.category}
                </span>
                ...
                <button
                  ...
                  className="ml-auto text-lg leading-none transition-colors hover:scale-110"
                  title={question.is_bookmarked ? 'Remove bookmark' : 'Bookmark this question'}
                >
                  <span style={{ color: question.is_bookmarked ? '#f59e0b' : 'rgba(255,255,255,0.15)' }}>
